...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "policyLinksFound") {
    ...
    if ((message.links.terms.length > 0 || message.links.privacy.length > 0) && 
    ...
    chrome.storage.local.get(['openaiApiKey'], async (result) => {
      if (result.openaiApiKey) {
        ...
        // Analyze the content
        analyzeContent(message.tabId, message.content, domain, result.openaiApiKey);
    ...
    if (hasContent) {
      ...
      try {
        ...
        if (openaiApiKey) {
          ...
          const { highRiskMatches, mediumRiskMatches, lowRiskMatches, initialRiskScore } = 
            performBasicAnalysis(lowerText);
          
          const fallbackAnalysis = createFallbackAnalysis(
            domain, highRiskMatches, mediumRiskMatches, lowRiskMatches, initialRiskScore
          );
...
function performBasicAnalysis(text) {
  ...
  const riskPatterns = {
    highRisk: [
      "sell your data", "share with third parties", "unlimited license", "no obligation to protect",
      "waive right to class action", "mandatory arbitration", "modify terms without notice",
  ...
  let initialRiskScore = (highRiskMatches.length * 15) + (mediumRiskMatches.length * 7) - (lowRiskMatches.length * 3);
  initialRiskScore = Math.max(0, Math.min(100, initialRiskScore));
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a legal expert that analyzes Terms & Conditions and Privacy Policies. Format your response strictly as JSON."
          },
          {
            role: "user",
            content: prompt
...
function createFallbackAnalysis(domain, highRiskMatches, mediumRiskMatches, lowRiskMatches, initialRiskScore) {
  // Create summary points based on pattern matches
  let summaryPoints = [];
  
  if (highRiskMatches.length > 0) {
    summaryPoints.push(`${domain} includes ${highRiskMatches.length} high-risk terms that may affect your privacy or rights.`);
  }
  
  if (mediumRiskMatches.length > 0) {
    summaryPoints.push(`Contains ${mediumRiskMatches.length} medium-risk terms related to data usage and tracking.`);
  }
  ...
  // Add general advice
  summaryPoints.push("Be cautious about how your data may be used or shared with third parties.");
  summaryPoints.push("Consider reviewing the full terms to understand all implications before agreeing.");
  
  // Build fallback analysis
  const fallbackAnalysis = {
    summary: `<ul>${summaryPoints.map(point => `<li>${point}</li>`).join('')}</ul>`,
  ...
  highRiskMatches.slice(0, 3).forEach(match => {
    fallbackAnalysis.riskFactors.push({
      ...
      description: "This pattern typically indicates higher risk to user privacy or rights.",
      level: "high"
    });
  });
